function inicializar(){
    ...
    var i, max, temp, contenido="", tdelement, tipo = "";
    ...
    do{
        tipo = prompt("El orden del arreglo es:", "");
        if(tipo != "ascendente" &&  tipo!="descendente"){
            alert("Porfavor ingrese \"ascendente\" o \"descendente\"");
        }
    }while(tipo != "ascendente" && tipo!="descendente");
    ...
    with(document){
        ...
        contenido += "<h1>Números ordenados "+tipo+"</h1>\n";
        contenido += "<table>\n\t<tbody>\n\t\t<tr>\n";
        //Lazo que muestra los elementos del arreglo que han sido
        //ordenados con el método de la burbuja
        if(tipo=="descendente"){
            var descendente = numeros.reverse();
            for(i=0; i<max; i++) {
                contenido += "\t\t\t<td class=\"Off\">" + descendente[i] + "</td>\n";
                }
        }else{
            for(i=0; i<max; i++) {
                contenido += "\t\t\t<td class=\"Off\">" + numeros[i] + "</td>\n";
